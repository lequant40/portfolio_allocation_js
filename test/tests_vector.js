...
QUnit.test('Vector basic manipulations', function(assert) {    
  ...
  var vect = new PortfolioAllocation.Vector(this.vectValues);
...
QUnit.test('Vector to array', function(assert) {    
  ...
  var vec = new PortfolioAllocation.Vector([1,2,3]);
  ...
  var nsVec = new PortfolioAllocation.Vector(this.vectValues);
  var nsVecArray = nsVec.toArray();
  assert.deepEqual(nsVecArray, this.vectValues, 'Array to vector to array');
  assert.equal(PortfolioAllocation.Vector.areEqual(nsVec, new PortfolioAllocation.Vector(nsVecArray)), true, 'Vector to array to vector');
});


QUnit.test('Vector dot product', function(assert) {    
  // Test vector dot product using static data
  var vec1 = new PortfolioAllocation.Vector([1,2,3]);
  var vec2 = new PortfolioAllocation.Vector([4,5,6]);
  assert.equal(PortfolioAllocation.Vector.dotProduct(vec1, vec2), 32, 'Vector dot product');
  ...
  var vec = new PortfolioAllocation.Vector(intArray);
  assert.equal(PortfolioAllocation.Vector.dotProduct(vec, vec), n_max*(n_max+1)*(2*n_max+1)/6, 'Vector dot product integer');
});
...
QUnit.test('Vector Hadamard product', function(assert) {    
  ...
  var vec1 = new PortfolioAllocation.Vector([1,2,3]);
  var vec2 = new PortfolioAllocation.Vector([1,2,3]);
  
  var hadamardVec = PortfolioAllocation.Vector.hadamardProduct(vec1, vec2);
  var expectedVec = new PortfolioAllocation.Vector([1,4,9]);
  
  assert.equal(PortfolioAllocation.Vector.areEqual(hadamardVec, expectedVec), true, 'Vector Hadamard product');
});
...
QUnit.test('Matrix-vector product', function(assert) {    
  ...
  var vec = new PortfolioAllocation.Vector([1,2,3]);
  var prodVec = PortfolioAllocation.Matrix.vectorProduct(mat, vec);

  var expectedResVec = new PortfolioAllocation.Vector([14,32]);
  assert.equal(PortfolioAllocation.Vector.areEqual(prodVec, expectedResVec), true, 'Matrix-vector product');
});
...
QUnit.test('Vector sum', function(assert) {    
  ...
  var vec = PortfolioAllocation.Vector.ones(n); 
...
QUnit.test('Vector normalisation', function(assert) {    
  // Static data
  var vec = new PortfolioAllocation.Vector([1,2,3]);
  var resvec = new PortfolioAllocation.Vector([1/6,2/6,3/6]);
  assert.equal(PortfolioAllocation.Vector.areEqual(vec.normalize(), resvec), true, 'Vector static normalized');

  // Vector of ones
  var n = Math.floor(Math.random()*(100-2+1) + 2); // min = 2, max = 100
  var vec = PortfolioAllocation.Vector.ones(n).normalize(); 
  for (var i = 1; i <= n; ++i) {
